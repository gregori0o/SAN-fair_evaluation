...
class SAN_EdgeLPE(nn.Module):
    def __init__(self, net_params):
        ...
        self.embedding_h = nn.Embedding(num_node_type, GT_hidden_dim)
        self.embedding_e = nn.Embedding(num_edge_type, GT_hidden_dim-LPE_dim)#Remove some embedding dimensions to make room for concatenating laplace encoding
        ...
        self.MLP_layer = MLPReadout(GT_out_dim, self.num_classes)        
    ...
    def loss(self, scores, targets):
        ...
        loss = nn.L1Loss()(scores, F.one_hot(targets.reshape(-1).long(), self.num_classes).float())
